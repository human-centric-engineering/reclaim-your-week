...
| Lead               | John                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Status             | **`RYW v1` in flight — F1–F6 shipped (F6 `ryw-current` #37).** Phase 0 setup + entitlement gate, Phase 1 bucket cards, and the `<ReclaimChart>` family are live; a leader can now start an audit, do Phase 0 and Phase 1, and see their week drawn back. **F7 `ryw-phases`** — Phases 2–6 (energy, ideal week, gap + refer-back, action plan, summary + share) — is now **in flight** and finishes the critical path. **F8 `ryw-access`** is `available` and layers invite tiers over F6's grant gate. _(Earlier detail: F4 landed the leaf schema, the single `saveAnswer` write-path (I3), the run lifecycle + reflection gate (I9/I15), and the consumer shell. F5 landed the optional `.ics` branch — in-memory parse, one `runStructuredCompletion` categorise, totals-only, the composite (I-composite), and `smoke:reclaim-calendar` proving no meeting title in the DB (I4). F6 landed Phase 0/1 + the chart family and the free-tier entitlement gate.)_ |
...
| F7  | `ryw-phases`       | John  | **in flight** | F6 (F5 optional) | 4      | Phases 2–6: energy, ideal week, gap + refer-back, action plan, summary + share                                |
...
**In flight:** **F7 `ryw-phases`** — F6 shipped (#37), so F7 is claimed and building (plan:
[[ryw-phases]]): Phases 2–6 — energy, ideal week, the gap analysis with the **refer-back** (I13), the
action plan, and the summary + share. It finishes the critical path. **F8 `ryw-access` is `available`
▲** and layers invite/referral tiers over F6's already-live free-tier grant gate (F6 t-1 bootstrapped
the gate; F8 adds tiers without changing it). F7 t-4's existing-client close coordinates with F8's
client tier.
...
_Owner:_ John · _Status:_ **in flight** · _Depends on:_ F6 (shipped #37) · F5 optional · _~4 tasks_ · detail: [[ryw-phases]]
