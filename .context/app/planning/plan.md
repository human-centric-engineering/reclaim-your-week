...
| Lead               | John                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Status             | **`RYW v1` in flight — F1–F8 and F10 shipped (F10 `ryw-admin` #43). Both sides of the product now exist: an invited leader consents, runs the audit Phase 0 → Phase 6 and leaves with a summary, and Rashmir can see who is in, where they stalled, what it cost, whether people come back, and what the cohort looks like anonymised — and can reword her own content without a deploy.** **F9 `ryw-repeat`** is now **in flight**, the **last feature in the epic**: trend lines over `getSlotHistory()`, the automatic comparative open, the recent-audit shortcut, the quarterly nudge, and relabelled buckets carried across runs. It is what makes a second audit worth doing, and therefore what Brief §1's success measure is actually measuring. _(Earlier detail: F4 landed the leaf schema, the single `saveAnswer` write-path (I3), the run lifecycle + reflection gate (I9/I15), and the consumer shell. F5 landed the optional `.ics` branch — in-memory parse, one `runStructuredCompletion` categorise, totals-only, the composite (I-composite), and `smoke:reclaim-calendar` proving no meeting title in the DB (I4). F6 landed Phase 0/1 + the chart family and the entitlement gate. F7 landed Phases 2–6, the refer-back (I13) as a run-scoped data flow, and the summary + optional share. F8 replaced F6's free-tier bootstrap with tiered invite redemption, the client window, the referral unlock and versioned consent. F10 landed the client list, the two success measures, the shared-results inbox + consent-filtered aggregate, the content editor, and the erasure proof that finally covers the audit answers themselves.)_ |
...
| F9  | `ryw-repeat`       | John  | **in flight** | F7               | 4      | Trend lines, comparative open, quarterly nudge, bucket relabelling                                            |
...
**In flight:** **F9 `ryw-repeat`** — **the last feature in the epic.** F10 shipped (#43), so both the
leader's side and the operator's side of v1 are done, and what is left is the thing the product is
ultimately for: Brief §1's success measure is whether people **come back**, and until F9 lands a
returning leader gets a blank second audit that knows nothing about their first. F9 is claimed and
building (plan: [[ryw-repeat]]): per-bucket trend lines over `getSlotHistory()` (the read F1 t-2
added for exactly this), the automatic comparative open, the recent-audit shortcut, the quarterly
nudge, and carrying relabelled bucket names across runs.

**It was passed over twice, deliberately, and that was right both times** — F8 made the invite gate
real and F10 made the list runnable, and neither could wait on a feature whose value only appears
once someone has two audits weeks apart. That reasoning also sets the honest expectation for F9
itself: **it can be built and tested now, but it cannot be _observed_ working until the cohort has
repeat data.** The plan's test strategy carries that weight instead of a smoke.
...
_Owner:_ John · _Status:_ **in flight** (F7 shipped #39) · _Depends on:_ F7 · _~4 tasks_ · detail: [[ryw-repeat]]
