...
| Lead               | John                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Status             | **`RYW v1` in flight — F1–F8 shipped (F8 `ryw-access` #41). The whole participant path exists and the door is now genuinely shut: an invited leader redeems a tiered invite, consents, and runs the audit end to end, Phase 0 → Phase 6, walking away with a summary; an account with no invite is refused at run creation.** **F10 `ryw-admin`** is now **in flight** — the operator half: the client list (abandonment, tier, qualification, cost per run), the two success measures the Brief names, the shared-results inbox + anonymised aggregate, content editing without a deploy, and the GDPR erasure proof. **F9 `ryw-repeat` is `available` ▲** and deliberately passed over until real users have two audits to compare. _(Earlier detail: F4 landed the leaf schema, the single `saveAnswer` write-path (I3), the run lifecycle + reflection gate (I9/I15), and the consumer shell. F5 landed the optional `.ics` branch — in-memory parse, one `runStructuredCompletion` categorise, totals-only, the composite (I-composite), and `smoke:reclaim-calendar` proving no meeting title in the DB (I4). F6 landed Phase 0/1 + the chart family and the entitlement gate. F7 landed Phases 2–6, the refer-back (I13) as a run-scoped data flow, and the summary + optional share. F8 replaced F6's free-tier bootstrap with tiered invite redemption, the client window, the referral unlock and versioned consent.)_ |
...
| F9  | `ryw-repeat`       | —     | available ▲   | F7               | 4      | Trend lines, comparative open, quarterly nudge, bucket relabelling                                            |
| F10 | `ryw-admin`        | John  | **in flight** | F7, F8           | 5      | Client list + access control, shared-results inbox, content editing, export + GDPR proof                      |
...
**In flight:** **F10 `ryw-admin`** — F8 shipped (#41), so the door is closed and the grant ledger,
tier and client flag its client list reads all exist. F10 is claimed and building (plan:
[[ryw-admin]]): the client list with abandonment and cost, the two success measures Rashmir named,
the shared-results inbox and anonymised aggregate, content editing without a deploy, and the GDPR
erasure proof. It is the **operator half** of v1 — everything built so far is what a leader sees;
this is what Rashmir sees. **F9 `ryw-repeat` is `available` ▲** and has been since F7 (#39); it was
passed over deliberately, because trend lines cannot be exercised until real users have completed two
audits weeks apart, and F10 is what makes the invite list runnable in the first place.
...
_Owner:_ — · _Status:_ available ▲ (F7 shipped #39) · _Depends on:_ F7 · _~4 tasks_
...
_Owner:_ John · _Status:_ **in flight** (F7 shipped #39, F8 shipped #41) · _Depends on:_ F7, F8 · _~5 tasks_ · detail: [[ryw-admin]]
