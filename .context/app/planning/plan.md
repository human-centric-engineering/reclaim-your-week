...
| Lead               | John                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Status             | **`RYW v1` in flight — F1 (#17), F2 (#19 #20 #21), F3 `ryw-firstlight` ★ (#25), F4 `ryw-shell` (#27–#30/#32), and F5 `ryw-calendar` (#34) shipped.** F4 landed the leaf schema, the single `saveAnswer` write-path (I3), the run lifecycle + reflection gate (I9/I15), and the consumer shell. F5 landed the optional `.ics` branch — in-memory parse, one `runStructuredCompletion` categorise, totals-only, the composite (I-composite), and `smoke:reclaim-calendar` proving no meeting title in the DB (I4). **F6 `ryw-current`** — Phase 0 setup + entitlement gate, Phase 1 bucket cards, and the `<ReclaimChart>` family — is now **in flight**; **F8 `ryw-access`** remains `available` and must land before F6's grant gate is fully real. |
...
| F6  | `ryw-current`      | John  | **in flight** | F4 (F8 for gate) | 4      | Phase 0 setup + entitlement gate; Phase 1 bucket cards + reflection; the chart family                         |
...
**In flight:** **F6 `ryw-current`** — F4 shipped (#27–#30/#32) and F5 `ryw-calendar` shipped (#34), so
F6 is claimed and building (plan: [[ryw-current]]): Phase 0 setup + the entitlement gate, Phase 1 bucket
cards + reflection, and the `<ReclaimChart>` family that plots F5's composite. **F8 `ryw-access` is
`available` ▲** and must land before F6's run-creation gate is fully real — F6 t-1 reads `ReclaimGrant`,
but grants are _created_ by F8, so F6 t-1 either bootstraps a free-tier grant or waits on F8 t-1/t-2
(the sequencing decision is in [[ryw-current]]). F7 stays `blocked → F6`.
...
_Owner:_ John · _Status:_ **in flight** · _Depends on:_ F4 (shipped #27–#30/#32) · F8 for the grant gate · _~4 tasks_ · detail: [[ryw-current]]
