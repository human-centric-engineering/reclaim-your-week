...
| Lead               | John                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Status             | **`RYW v1` in flight — F1–F7 shipped (F7 `ryw-phases` #39). The critical path is complete: an invited leader can run the whole audit end to end, Phase 0 → Phase 6, and walk away with a summary.** **F8 `ryw-access`** is now **in flight** — it closes the door the product claims is already shut (self-signup is open today and F6 bootstraps a free grant to anyone who asks), lands the tiered invite → grant flow, the referral unlock, and the consent record F10's aggregate analysis needs as its lawful basis. **F9 `ryw-repeat` is `available` ▲**; F10 `ryw-admin` waits on F8. _(Earlier detail: F4 landed the leaf schema, the single `saveAnswer` write-path (I3), the run lifecycle + reflection gate (I9/I15), and the consumer shell. F5 landed the optional `.ics` branch — in-memory parse, one `runStructuredCompletion` categorise, totals-only, the composite (I-composite), and `smoke:reclaim-calendar` proving no meeting title in the DB (I4). F6 landed Phase 0/1 + the chart family and the free-tier entitlement gate. F7 landed Phases 2–6, the refer-back (I13) as a run-scoped data flow, and the summary + optional share.)_ |
...
| F8  | `ryw-access`       | John  | **in flight** | F4               | 4      | Tiered invites, the grant ledger, referral unlock (gates F6's run creation)                                   |
| F9  | `ryw-repeat`       | —     | available ▲   | F7               | 4      | Trend lines, comparative open, quarterly nudge, bucket relabelling                                            |
| F10 | `ryw-admin`        | —     | blocked → F8  | F7, F8           | 5      | Client list + access control, shared-results inbox, content editing, export + GDPR proof                      |
...
**In flight:** **F8 `ryw-access`** — F7 shipped (#39) and the critical path is done, so F8 is claimed
and building (plan: [[ryw-access]]): the tiered invite → grant flow, the referral unlock, and the
consent record. It is the feature that makes "invite-gated" true — **today it is not**: Sunrise's
self-signup is open and F6's gate bootstraps a free grant to any account that asks for a run, so the
free tier is currently self-serve. F8 layers tiers over that same gate (I14) rather than moving it.
**F9 `ryw-repeat` is `available` ▲** (F7 shipped); **F10 `ryw-admin`** stays blocked on F8, whose
grant ledger and client flag are half of what its client list reads.
...
_Owner:_ John · _Status:_ **in flight** · _Depends on:_ F4 (shipped) · _~4 tasks_ · detail: [[ryw-access]]

Parallels F4–F6. Must land before F6's run-creation gate is real — **it is not real yet**: self-signup
is open and `assertEntitled` bootstraps a free grant for any account on first run.
...
_Owner:_ — · _Status:_ blocked → F8 (F7 shipped #39) · _Depends on:_ F7, F8 · _~5 tasks_
