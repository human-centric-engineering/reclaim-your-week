...
| `tests/unit/lib/email/registry.test.tsx`                       | Stubs the email seam empty in the fallback case          | —            | Keep ours. The test is named for the no-override path; this app overrides `welcome`, so reading the live seam makes it assert our copy |
| `tests/unit/lib/auth/config-database-hook.test.ts`             | Same stub, so the hook's spies see the platform template | —            | Keep ours. Without it the override renders, the spies never fire, and the failures read as "the hook did not send"                     |

The two test rows have **no upstream ask**: they are not carried features, just two Sunrise tests that assumed `welcome` was never overridden. Any fork that overrides it hits the same thing, so re-apply the stub and move on.
